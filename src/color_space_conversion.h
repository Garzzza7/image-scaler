...
inline void
YCbCr_to_RGB_Convert(unsigned char *src, const int &height, const int &width, const int &channels) {
    int chan{0};

    if (channels < 3) {
        std::cout << "IMAGE HAS LESS THAN 3 CHANNELS." << '\n';
        exit(1);
    }
...
inline void
RGB_to_YCbCr_Convert(unsigned char *src, const int &height, const int &width, const int &channels) {
    int chan = 0;
