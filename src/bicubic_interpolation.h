...
inline void
BiCubic_Filter(std::vector<unsigned char> &pix,
	       const std::vector<unsigned char> &src, const int height,
...
inline void
Resize_Image_BiCubic_Interpolation(std::vector<unsigned char> &src,
				   const int height, const int width,
				   const int channels, const int resize_height,
				   const int resize_width, unsigned char *res) {
    const double ratio_height
	= static_cast<double>(resize_height) / static_cast<double>(height);
    const double ratio_width
	= static_cast<double>(resize_width) / static_cast<double>(width);
    std::vector<unsigned char> pix(100);

    int k = 0;
    for (int i = 0; i < resize_height; i++) {
	const double src_y
	    = (static_cast<double>(i) + 0.5f) / ratio_height - 0.5f;
	for (int j = 0; j < resize_width; j++) {
	    const double src_x
		= (static_cast<double>(j) + 0.5f) / ratio_width - 0.5f;
