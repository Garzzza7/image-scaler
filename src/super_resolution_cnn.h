...
inline void
Convolution_99x11x55_offset(std::vector<unsigned char> &src, std::vector<double> &cnndst, const int &height, const int &width,
                            const int &offset, const double &partcnn) {
    ...
    for (int row = offset - 2; row < height + 2 - offset; row++) {
        for (int col = offset - 2; col < width + 2 - offset; col++) {
            for (int k = 0; k < CONV_LAYER1_FILTERS; k++) {
                ...
                temp_buffer[k] = (temp_buffer[k] < 0.0f) ? 0.0f : temp_buffer[k];
            }
...
inline void
SRCNN_Block(unsigned char *src, std::vector<unsigned char> &block, std::vector<double> &cnndst, const int &height,
            const int &width, const int &channels, const int &block_size, const int &overlap, const double &partcnn) {
    int xf, yf;
    int ki{0};
    int k{0};
